...
def main():
    ...
    for frame in container.decode(video=0):
        ...
        out_max_contour = max(c_c,key=cv2.contourArea)
	red_rect = cv2.minAreaRect(out_max_contour)
  	#cv2.rectangle(show_image,(int(red_rect[0][0]),int(red_rect[0][1])),(int(red_rect[0][0])+int(red_rect[1][0]),int(red_rect[0][1])+int(red_rect[1][1])),(255,0,0),2)
	#red_center = [red_rect[0][0]+red_rect[1][1]/2,red_rect[0][1]+red_rect[1][0]/2]
	
	cv2.circle(show_image,(int(red_rect[0][0]),int(red_rect[0][1])),15,(255,0,0),-1)
	cv2.putText(show_image,str(red_rect[1][1]),(10,30),cv2.FONT_HERSHEY_COMPLEX,2.0,(0,255,0),3)       
	if red_rect[1][1]<600:
	  pub.publish(test([red_rect[0][0],red_rect[0][1],1]))
	else:
	  pub.publish(test([red_rect[0][0],red_rect[0][1],-1]))
